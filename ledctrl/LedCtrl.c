...
unsigned short iLedControlValue=0xFFFF;
...
void led_control(unsigned short iLedStatus,unsigned short iLedBitMask)
{
	int lt_fd;
	lt_fd = open(HC595_LT_VAL,O_WRONLY | O_SYNC);
	write(lt_fd,GPIO_PIN_OUTPUT_0,sizeof(GPIO_PIN_OUTPUT_0));
...
void led_lte_ctrl(void)
{
	...
	return ;
	//led_control(LED_3G_OFF,LED_LTE_BIT_MASK);
...
int main(void)
{
	...
	//led_control(LED_WIFI1_GREEN_ON,LED_WIFI1_BIT_MASK);
	//led_control(LED_WIFI2_GREEN_ON,LED_WIFI2_BIT_MASK);
	//led_control(0x0 << 11,0x1 << 10);

	//ledd_control(LED_LTE_NO_SIM,LED_LTE_BIT_MASK);0x1 << 11
	//led_control(LED_CLOUD_RED_ON,LED_CLOUD_BIT_MASK);
	//led_nmea_status=LED_NMEA_BUS_NO_DATE;
	//led_lte_status=LED_LTE_NO_SIM;
	//led_cloud_status=LED_CLOUD_NO_CONNECTED;
